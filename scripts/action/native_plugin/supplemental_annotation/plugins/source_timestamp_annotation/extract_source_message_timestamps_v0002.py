...
# -*- coding: utf-8 -*-
# ============================================================
# 文件名: extract_source_message_timestamps_v0002.py
# 中文名: 已授权规范来源消息时间提取脚本
# 版本号: v0002
#
# 主层级: action
...
# ALIAS_META
# ============================================================
# alias: extract_source_message_timestamps_v0002
# family: extract_source_message_timestamps
# role: authorized_source_message_timestamp_extractor
# version: v0002
# status: experimental
# entry_point: scripts/action/native_plugin/supplemental_annotation/plugins/source_timestamp_annotation/extract_source_message_timestamps_v0002.py
# input:
...
SCRIPT_NAME = "extract_source_message_timestamps_v0002.py"
...
def _load_allowed(path: Path) -> Tuple[Set[Tuple[str, str, int, str]], str]:
    ...
    with path.open("r", encoding=DEFAULT_ENCODING) as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            try:
                ...
                raise AuthorizedTimestampExtractionError(f"allowed coordinates line {line_number} is invalid JSON: {exc}") from exc
            if not isinstance(row, dict):
                raise AuthorizedTimestampExtractionError(f"allowed coordinates line {line_number} must be an object")
            asset_id = row.get("asset_id")
            path_value = row.get("path")
            value_index = row.get("value_index")
            source_sha1 = row.get("source_value_sha1")
            if not isinstance(asset_id, str) or not asset_id or not isinstance(path_value, str) or not path_value:
                raise AuthorizedTimestampExtractionError(f"allowed coordinates line {line_number} has invalid identity")
            if isinstance(value_index, bool) or not isinstance(value_index, int) or value_index < 0:
                raise AuthorizedTimestampExtractionError(f"allowed coordinates line {line_number} has invalid value_index")
            if not isinstance(source_sha1, str) or not source_sha1:
                raise AuthorizedTimestampExtractionError(f"allowed coordinates line {line_number} has invalid source_value_sha1")
            coordinate = (asset_id, path_value, value_index, source_sha1)
            if coordinate in allowed:
                raise AuthorizedTimestampExtractionError(f"allowed coordinates contain duplicate at line {line_number}")
    ...
    if len(asset_ids) != 1:
        raise AuthorizedTimestampExtractionError("source timestamp extraction currently requires exactly one authorized asset")
    return allowed, asset_ids[0]


def _select_created_fact(message: Mapping[str, Any]) -> Tuple[Optional[Mapping[str, Any]], str, Optional[str]]:
    facts = [fact for fact in message.get("event_times", []) if isinstance(fact, Mapping) and fact.get("event_kind") == EVENT_TIME_KIND]
    verified = [fact for fact in facts if fact.get("semantic_status") == "verified" and isinstance(fact.get("normalized_utc"), str)]
    if len(verified) == 1:
        return verified[0], "resolved", None
    if len(verified) > 1:
        unique = {(str(fact.get("normalized_utc")), _canonical_json(fact.get("source_ref"))) for fact in verified}
        if len(unique) == 1:
            return verified[0], "resolved", None
...
def extract_authorized(access_manifest_path: Path, project_root: Path) -> Dict[str, Any]:
    ...
    if not isinstance(canonical_ref, Mapping) or not isinstance(allowed_ref, Mapping):
        raise AuthorizedTimestampExtractionError("source access manifest is missing canonical source or allowed coordinates")
    ...
    if source.get("schema_version") != CANONICAL_SCHEMA_VERSION:
        raise AuthorizedTimestampExtractionError("authorized source is not canonical_conversation_ingress_v0001")
    ...
    for conversation_index, conversation in enumerate(source.get("conversations", [])):
        ...
        for node_index, node in enumerate(conversation.get("nodes", [])):
            ...
            for part_index, part in enumerate(parts if isinstance(parts, list) else []):
                if not isinstance(part, Mapping) or part.get("part_type") != "text" or not isinstance(part.get("text"), str):
                    continue
                text = str(part["text"])
                ...
                if coordinate in matching_allowed:
                    ...
                    event_time = fact.get("normalized_utc") if status == "resolved" and isinstance(fact, Mapping) else None
                    identity_payload = {
                    ...
                    annotation = {
                        ...
                        "source_text_path": part.get("source_ref", {}).get("source_path") if isinstance(part.get("source_ref"), Mapping) else None,
                ...
                value_index += 1
    if not annotations:
        raise AuthorizedTimestampExtractionError("no authorized parsed canonical text coordinates produced timestamp annotations")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Extract message timestamps only from a source authorized after structural parsing.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        if annotations_path.exists() or issues_path.exists() or manifest_path.exists():
            raise AuthorizedTimestampExtractionError("timestamp extraction outputs are append-only and must not already exist")
