#!/usr/bin/env python3
# ============================================================
# File: analyze_structural_cooccurrence_v0004.py
# 中文名: 结构共现分析脚本
# Version: v0004
# Layer: execution
# Main Layer: action
...
# alias: analyze_structural_cooccurrence_v0004
# family: analyze_structural_cooccurrence
# role: structural_cooccurrence_analysis
# version: v0004
# status: active
# entry_point: scripts/action/pipelines/analyze_structural_cooccurrence_v0004.py
# input:
...
SCRIPT_NAME = "analyze_structural_cooccurrence_v0004.py"
...
def analyze_cooccurrence(
    ...
    meta = {
        ...
        "known_limitations": [
            "v0004 outputs per-container pairs without global aggregation; aggregation is builder's responsibility.",
            "v0004 requires unit_id and unit_text in input; rows missing these fields are skipped.",
            "v0004 cooccurrence_count is always 1 per output row (one pair per container occurrence).",
        ],
        "v0003_changes": [
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze structural cooccurrence at segment level (v0004).",
    )
    ...
    parser.add_argument("--output", default="actioning/pipelines/analyze_structural_cooccurrence_action/structural_cooccurrence.jsonl", help="Output cooccurrence JSONL path (single-file mode).")
    parser.add_argument("--output-dir", default="actioning/pipelines/analyze_structural_cooccurrence_action", help="Output directory for per-file cooccurrence JSONL (directory mode).")
    parser.add_argument("--run-meta", default="actioning/pipelines/analyze_structural_cooccurrence_action/run_meta.json", help="Run meta JSON output path (single-file mode).")
    parser.add_argument("--run-meta-dir", default="actioning/pipelines/analyze_structural_cooccurrence_action", help="Output directory for per-file run_meta JSON (directory mode).")
