...
# - 单条调用保持原事务；write_many 按有界批次提交，失败回退当前批，提交后才返回结果
#
# 可更新: True
# ============================================================
...
# ALIAS_META
# ============================================================
# alias: sql_writer_v0005
# family: sql_writer
# role: sql_write_gateway
# version: v0005
# status: active
# entry_point: scripts/action/anchor/sql_writer_v0005.py
# input:
...
class SqlWriter:
    def __init__(self, config_path: str | Path, *, strict: bool = True, verify_schema_on_connect: bool = True):
        self._batch_connection: Optional[sqlite3.Connection] = None
    ...
    def _commit_call(self, conn: sqlite3.Connection) -> None:
        if self._batch_connection is None:
            conn.commit()

    def _close_call(self, conn: sqlite3.Connection) -> None:
        if self._batch_connection is None:
            conn.close()

    def write_many(self, object_type: str, records: Iterable[Dict[str, Any]], *, batch_size: int = 500) -> Iterator[WriteResult]:
        """Yield results only after each bounded transaction commits.

        Single-object APIs retain their original transaction semantics. A failed
        chunk rolls back in full; earlier committed chunks remain auditable.
        One writer instance must not be shared between concurrent executions.
        """
        if object_type not in {"concept", "instance"}:
            raise DataError("write_many supports concept and instance only")
        if isinstance(batch_size, bool) or not isinstance(batch_size, int) or batch_size < 1:
            raise DataError("batch_size must be a positive integer")
        if self._batch_connection is not None:
            raise DataError("Nested batch writes are not supported")
        source = iter(records)
        while True:
            chunk = list(islice(source, batch_size))
            if not chunk:
                return
            conn = self.connect()  # Schema and PRAGMAs checked once per connection.
            results = []
            try:
                conn.execute("BEGIN IMMEDIATE")
                self._batch_connection = conn
                method = self.write_concept if object_type == "concept" else self.write_instance
