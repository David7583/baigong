# ============================================================
# 文件名: sync_sql_to_duckdb_v0006.py
# 中文名: 完整快照分块写入 DuckDB
# 版本号: v0006
#
# 主层级: action
# 层级: derivation / synchronization
# 脚本定位: 将 SQL 真源分块追加到分析库
#
# 职责说明:
# - 保留完整快照，采用列数组批量参数写入并核对行数
# 本脚本做什么:
# - 追加写入同步表，保留原始 run_id 与审计信息
# 本脚本不做什么:
# - 不修改 SQLite 真源，不删除历史同步记录，不改变对象身份
# 制度边界声明:
# - 不筛选来源；不改变 latest 快照语义；不安装新依赖
# 可更新: True
# ============================================================
# ALIAS_META
...
# version: v0006
# status: active
# entry_point: scripts/action/derivation/sync_sql_to_duckdb_v0006.py
# input:
#   - all SQL records and existing DuckDB
# output:
#   - DuckDB append rows and sync evidence
# depends_on:
#   - sqlite3, PyYAML, DuckDB
# used_by:
#   - action_derivation_materialization_pipeline_v0006
# ============================================================
...
SCRIPT_NAME = "sync_sql_to_duckdb_v0006.py"
SCRIPT_VERSION = "v0006"
SCRIPT_FAMILY = "derivation_sync"
...
def _chunked_sync(
    ...
    insert_sql = (f"INSERT INTO {duckdb_table} ({fields_str}) SELECT "
    ...
    try:
        ...
        column_types = {row[1]: row[2] for row in
                        duck_conn.execute(f"PRAGMA table_info('{duckdb_table}')").fetchall()}
        batch_schema = json.dumps([{field: column_types[field] for field in duckdb_fields}])
        ...
        while True:
            ...
            # Preserve source column order in the bounded batch.
            batch = []
            for row in raw_chunk:
                values = [sync_id, synced_at] + [row[f] for f in sql_fields]
                batch.append(values)

            # Per-chunk transaction; strict native conversion preserves types and NULLs.
            try:
                payload = json.dumps([dict(zip(duckdb_fields, row)) for row in batch],
                                     ensure_ascii=False, allow_nan=False)
                duck_conn.execute("BEGIN TRANSACTION")
                duck_conn.execute(insert_sql, [payload, batch_schema])
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Sync full SQL fact snapshots to DuckDB (append-only), using bounded strict batch conversion."
    )
    ap.add_argument(
        "--target",
